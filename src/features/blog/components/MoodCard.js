...
function MoodCard({ post, onEdit, onDelete }) {
  ...
  const coverUrl = post.coverImage || 'https://images.unsplash.com/photo-1501594907352-04cda38ebc29?w=800';
    ...
    <Card className="group relative overflow-hidden hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-700">
      ...
      <CardContent className="p-6 space-y-4">
        ...
        <h3 
          ...
          className="text-2xl font-bold text-center cursor-pointer hover:text-orange-600 dark:hover:text-orange-400 transition-colors line-clamp-2 min-h-[4rem] flex items-center justify-center"
        >
          {post.title || '未命名'}
        </h3>
        ...
        <div 
          onClick={handleViewDetail}
          className="relative w-full aspect-video rounded-lg overflow-hidden cursor-pointer group-hover:opacity-95 transition-opacity bg-gradient-to-br from-orange-50 to-yellow-50 dark:from-orange-900 dark:to-yellow-900"
        >
          <img
            src={coverUrl}
            alt={post.title}
            ...
            onError={(e) => {
              e.target.src = 'https://images.unsplash.com/photo-1501594907352-04cda38ebc29?w=800';
            }}
          />
          {/* 图片遮罩 */}
        ...
        <div className="flex justify-center">
          <Button
            onClick={handleViewDetail}
            className="px-8 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-medium"
            size="sm"
          >
            阅读全文
          </Button>
      ...
      <RoleGuard roles={['editor', 'admin']}>
        <CardFooter className="pt-0 px-6 pb-4 flex gap-2">
          <Button
            ...
            size="sm"
            onClick={() => onEdit(post)}
            className="flex-1 gap-1.5 hover:bg-orange-50 hover:text-orange-600 hover:border-orange-300 transition-colors"
          >
